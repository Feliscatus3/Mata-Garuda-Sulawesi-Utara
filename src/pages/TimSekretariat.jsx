  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.error('Gagal memuat data tim sekretariat:', error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="pt-32 lg:pt-44 pb-24 font-urbanist bg-[#FDFDFD] min-h-screen">
        <div className="max-w-[1440px] mx-auto px-5 lg:px-[60px] text-center">
          <p className="text-gray-500 font-medium">Memuat data tim sekretariat...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-[1440px] mx-auto px-5 lg:px-[60px]">
        ...
        >
          <div className="flex items-center gap-2 text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">
            ...
            <span className="hover:text-[#B8860B] cursor-pointer" onClick={() => navigate('/organisasi/pengurus')}>Organisasi</span>
            <ChevronRight size={14} />
            <span className="text-[#B8860B]">Tim Sekretariat</span>
          </div>
          <h1 className="text-[40px] lg:text-[56px] font-[900] text-black leading-none tracking-tight">
            Tim <span className="text-[#B8860B]">Sekretariat</span>
          </h1>
          <div className="w-20 h-1.5 bg-[#B8860B] mt-6 rounded-full"></div>
          <p className="mt-8 text-gray-500 max-w-2xl font-medium text-lg">
            Tim sekretariat Mata Garuda Sulawesi Utara yang bertugas mengelola administrasi dan operasional organisasi.
          </p>
        </motion.div>
          ...
          {data.map((item) => (
            ...
            >
              ...
              <div className="relative aspect-[3/4] overflow-hidden bg-gray-50">
                ...
                {/* Overlay on Hover */}
                <div className="absolute inset-0 bg-gradient-to-t from-[#B8860B]/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end justify-center pb-8">
                  {item.email && (
                    <a 
                      href={`mailto:${item.email}`}
                      className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-[#B8860B] hover:bg-[#B8860B] hover:text-white transition-all shadow-lg"
              ...
              <div className="p-5 text-center flex flex-col items-center">
                ...
                <div className="h-[48px] flex items-center justify-center mb-2">
                  <span className="text-[15px] lg:text-[16px] font-black text-[#B8860B] uppercase tracking-normal opacity-80 leading-tight">
                    {item.posisi 
                      ? item.posisi
                ...
                <div className="h-[52px] flex items-center justify-center w-full mb-1">
                  <h3 className="text-[18px] lg:text-[20px] font-[800] text-gray-900 leading-tight group-hover:text-[#B8860B] transition-colors">
                    {item.nama}
                  </h3>
                </div>
                ...
                <div className="w-6 h-[2px] bg-gray-200 mx-auto group-hover:w-12 group-hover:bg-[#B8860B] transition-all duration-500"></div>
              </div>
            </motion.div>
          ))}
        </motion.div>
