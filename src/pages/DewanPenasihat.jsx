  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.error('Gagal memuat data dewan penasihat:', error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="pt-32 lg:pt-44 pb-24 font-urbanist bg-[#FDFDFD] min-h-screen">
        <div className="max-w-[1440px] mx-auto px-5 lg:px-[60px] text-center">
          <p className="text-gray-500 font-medium">Memuat data dewan penasihat...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-[1440px] mx-auto px-5 lg:px-[60px]">
        ...
        >
          <div className="flex items-center gap-2 text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">
            ...
            <ChevronRight size={14} />
            <span className="text-[#B8860B]">Dewan Penasihat</span>
          </div>
          <h1 className="text-[40px] lg:text-[56px] font-[900] text-black leading-none tracking-tight">
            Dewan <span className="text-[#B8860B]">Penasihat</span>
          </h1>
          <div className="w-20 h-1.5 bg-[#B8860B] mt-6 rounded-full"></div>
          <p className="mt-8 text-gray-500 max-w-2xl font-medium text-lg">
            Dewan penasihat Mata Garuda Sulawesi Utara yang memberikan arahan dan bimbingan strategis dalam menjalankan misi organisasi.
          </p>
        </motion.div>
            ...
            <div key={category.id} className="mb-20">
              <div className="flex flex-col mb-10">
                <h2 className="text-[24px] lg:text-[32px] font-[900] text-black tracking-tight">
                  Kategori <span className="text-[#B8860B]">{category.title}</span>
                </h2>
                  ...
                  >
                    ...
                    <div className="relative aspect-[3/4] overflow-hidden bg-gray-50">
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-[#B8860B]/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end justify-center pb-8">
                        {guru.email && (
                          <a href={`mailto:${guru.email}`} className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-[#B8860B] hover:bg-[#B8860B] hover:text-white transition-all shadow-lg">
                            <Mail size={18} />
                          </a>
                        )}
                      </div>
